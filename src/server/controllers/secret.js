...
const DEFAULT_EXPIRE = 60 * 60 * 24 * 1000;
...
async function secret(fastify) {
    ...
    fastify.post(
        ...
        async (req, reply) => {
            const { text, title, password, ttl, allowedIp, preventBurn, maxViews } = req.body;
            ...
            await prisma.secret.create({
                data: {
                    ...
                    expiresAt: new Date(
                        Date.now() + (parseInt(ttl) ? parseInt(ttl) * 1000 : DEFAULT_EXPIRE)
                    ),
                },
            });
