...
export function FileUpload({ onFileChange }: { onFileChange: (files: File[]) => void }) {
    ...
    if (!user) {
        return (
            <div className="border-2 border-dashed border-dark-500/50 p-4 text-center bg-dark-700/20 hover:border-teal-500/50 transition-all duration-300 group">
                <div className="flex items-center justify-center space-x-3">
                    <div className="p-2 bg-dark-600/50 group-hover:bg-teal-500/20 transition-all duration-300">
                        <Lock className="w-4 h-4 text-slate-400 group-hover:text-teal-400 transition-colors duration-300" />
                    </div>
                    ...
                    <button className="px-3 py-1.5 bg-teal-500 hover:bg-teal-600 text-white text-sm transition-all duration-300 hover:scale-105 hover:shadow-lg">
    ...
    return (
        <div>
            <div
                {...getRootProps()}
                className={`border-2 border-dashed p-4 text-center transition-all duration-300 group ${isDragActive ? 'border-teal-500/80 bg-dark-700/40' : 'border-dark-500/50 bg-dark-700/20 hover:border-teal-500/50'}`}
            >
                <input {...getInputProps()} />
                <div className="flex flex-col items-center justify-center space-y-2">
                    <UploadCloud className="w-8 h-8 text-slate-400 group-hover:text-teal-400 transition-colors duration-300" />
                    {isDragActive ? (
                        <p className="text-slate-300 text-sm">{t('file_upload.drop_files_here')}</p>
                    ) : (
            ...
            {files.length > 0 && (
                <div className="mt-3 space-y-2">
                    {files.map((file, index) => (
                        <div key={index} className="flex items-center justify-between bg-dark-700/50 p-2">
